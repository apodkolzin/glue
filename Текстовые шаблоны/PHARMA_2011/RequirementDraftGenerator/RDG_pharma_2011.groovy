...
//zft = helper.get('reqdrafs000080000k29kd2i4n7n5hrg')
helper.execute { s ->
...
for (SimpleCatalogItem item in scienceProgramFormSet)
{
	...
	if (!item.title.equals("другое")) scienceProgramFormLength++
}
...
StringBuilder profEducationBuilder = new StringBuilder()
for(def item in zft.educationLevelSet){
    profEducationBuilder.append(item.title).append(";\n")
}
report.vars.profEducationLevel = profEducationBuilder

StringBuilder programTypeBuilder = new StringBuilder()
for(def item in zft.farmaProgramTypeSet){
    programTypeBuilder.append(item.title).append(";\n")
}
