...
demand = BusinessActionBase.ensureReload(session, contract.getDemand())
demandNumber = demand.getViewNumber() //карточка заявки дочерняя выбранному контракту  поле  «Номер»

def code = "1"
ownFunds = getBoxValueByAxis(code, contractReport)//Собственные средства организации
code = "2"
credits = getBoxValueByAxis(code, contractReport)//Кредиты
code = "3"
foreignInvestorFunds = getBoxValueByAxis(code, contractReport)//Средства иностранных инвесторов
code = "4"
borrowingCosts = getBoxValueByAxis(code, contractReport)//Заемные средства
code = "5"
otherAssets = getBoxValueByAxis(code, contractReport)//Прочие
...
BigDecimal totalAttractedValue = BigDecimal.ZERO;
  for (ContractStage stage : ContractStageHibernateHandler.listContractStages(contract)) {
      report.vars.stageNumber = stage.getNumber()//карточка этапа дочерняя выбранному контракту поле «Номер»;
...
plannedValue = ContractBudgetUtil.getOffBudget4Year(contract, year);//карточка отчета, поле «Запланировано, рублей» (брать за год родительского этапа поле "Дата окончания")
attractedValue = attractedValuesByYears.get(year);//карточка отчета, поле «Привлечено, рублей» (брать за год родительского этапа поле "Дата окончания")
//
...
BigDecimal plannedAttractedDifference = plannedValue - attractedValue
...
BigDecimal sumPlanned = 0
BigDecimal sumAttracted = 0
...
for (Integer yearForSum : contractYears) {
    if(null != ContractBudgetUtil.getOffBudget4Year(contract, yearForSum)){
        sumPlanned = sumPlanned + ContractBudgetUtil.getOffBudget4Year(contract, yearForSum)
    }
    if(null != attractedValuesByYears.get(yearForSum)){
        sumAttracted = sumAttracted + attractedValuesByYears.get(yearForSum)
    }
}


BigDecimal plannedValueMinusOneYear = sumPlanned - plannedValue
BigDecimal attractedValueMinusOneYear = sumAttracted - attractedValue

/*
BigDecimal plannedValueMinusOneYear = ContractBudgetUtil.getOffBudget4Year(contract, year - 1)//карточка отчета, поле «Запланировано»  (брать за предшествующий  год родительского этапа, поле "Дата окончания")
...
BigDecimal sumPlanned = plannedValueMinusOneYear + plannedValue//Сумма полей карточки отчета: «Запланировано, рублей»  (брать за предшествующий  год родительского этапа, поле "Дата окончания"); «Запланировано, рублей»  (брать за год родительского этапа поле "Дата окончания").
BigDecimal sumAttracted = attractedValueMinusOneYear + attractedValue//Сумма полей карточки отчета:   «Привлечено , рублей»  (брать за предшествующий  год родительского этапа, поле "Дата окончания");«Привлечено, рублей»  (брать за год родительского этапа поле "Дата окончания")
*/

BigDecimal plannedAttractedDifferenceTotal = sumPlanned - sumAttracted 
...
//report.vars.stageNumber = stage.getNumber()
...
report.vars.attractedValueMinusOneYear = attractedValueMinusOneYear
...
def getBoxValueByAxis(code, contractReport){
