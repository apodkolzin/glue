...
{
def q = s.createQuery("select docVersion from ContractProjectDocument where parent.id='$parent.UUID' and BOCase.code='$caseCode' order by docVersion desc")
q.setMaxResults(1)
...
stage = boCase.workflowDefinition.getStageByIdentificator("2", s)
new AddFcntpStageWorkResultBA(ps(["identifier":'1', "title":boCase.title, "docVersion":docVersion,"activeVersion":true, "category":boCase, "currentStage":stage]), user, parent).execute(session)
updateObject(subject, ps(["activeVersion":false]), s)
}

createDoc("contractProject", session);