...
'body': 'RemarkOwnerDocumentLetter/body_creative4']

/**
* Methods -----------------------------------------------------------------------------------------------------------
*/
def getContractDocs = {contract, ccamBOCase ->
q=session.createQuery("FROM ru.naumen.fcntp.bobject.document.FcntpDocumentBase x0 WHERE x0.parent=:parent AND x0.ccamBOCase.UUID IN('"+ccamBOCase.UUID+"')");
q.setParameter('parent',contract);
res=q.list();
...
'UUID':getContractDocs(contract, ru.naumen.core.catalogsengine.CoreCatalogHibernateHandler.getCatalogItemByCode('CCAMDocumentCases', 'documentContract_Creative')).UUID,
