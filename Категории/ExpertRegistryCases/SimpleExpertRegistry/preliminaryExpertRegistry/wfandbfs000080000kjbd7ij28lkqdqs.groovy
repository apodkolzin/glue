...
if (helper.get(expertRegistry.UUID) == null)
{
    return
}
...
def getExperts(lot, registryTypes, session)
{
    ...
    query.setParameterList("priorLines", lot.subPriorityLines.collect({
        it.UUID
    }))
...
def validate(lot)
{
    ...
    {
        throw new UIForm.UIFormUserException("Формирование Предварительного реестра экспертов невозможно. Необходимо заполнить блок 'Рубрикатор направлений исследования' Лота.")
    }
    if (lot.subPriorityLines.isEmpty())
    {
        throw new UIForm.UIFormUserException("Формирование Предварительного реестра экспертов невозможно. Необходимо заполнить блок 'Направление и технологии' Лота.")
    }
}
