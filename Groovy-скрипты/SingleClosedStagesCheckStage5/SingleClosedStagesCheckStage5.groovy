...
{
   def lastStage = getStage(contract)
    //проверяем, послано ла уже сообщение по этому контракту
   if (lastStage == null)
       continue
   else
       if((helper.query("select l from CoreLogEvent l where l.subjectUUID = '" + lastStage.UUID + 
...
def getStage(def contract)
{
    stages = ContractStageHibernateHandler.listContractStages(contract)
    def lastStage
    for (stage in stages)
    {
        if(stage.currentStage?.identificator.equals("4"))
            lastStage = stage
    }
    return lastStage
}