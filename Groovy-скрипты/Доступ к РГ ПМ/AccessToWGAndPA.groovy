 ...
 */
	...
	queries = new Queries3(session, programUUID)
	...
	for (def EmployeeStech memberEmployee in employees)
	{
	    ...
	    q = queries.progActions
	    q.setParameter("memberId", memberEmployee.UUID)
	    
        def List<String> progActions = q.list()
                
		q = queries.workGroupsByProgram
		q.setParameter("memberId", memberEmployee.UUID)
		
	    def List<String> workGroupsByProgram = q.list()
        
	    
	    if(progActions.size()==0 && workGroupsByProgram.size()==0){
	    	continue
	    }
...
class Queries3{
    Query progActions
    Query workGroupsByProgram
    Query roleRels

    Queries3(session, programId){
    	progActions = session.createQuery("select rel.rightBO.persistedDisplayableTitle from " + PersonProgramActionRelation.class.getName() + 
        ...
        progActions.setParameter("programId", programId)
        
        workGroupsByProgram = session.createQuery("select rel.rightBO.title from PersonWorkgroupRelation as rel where rel.leftBO.id = :memberId " +
        ...
        workGroupsByProgram.setParameter("programId", programId)
        
        roleRels = session.createQuery("select rcRel FROM RoleCapabilityRelation rcRel WHERE rcRel.rightBO.Id in ('workGroupSecretary', 'workGroupMember') and rcRel.leftBO.id = :memberId")
    }
}