package resources.groovy.mon.contractsWithDocumentAgreement
...
{
    def documents = getDocumentAgreement(contract.UUID)
   ...
   if((helper.query("select l from CoreLogEvent l where l.subjectUUID = '" + contract.UUID + 
        ...
        date.minus(1) + "'").size() == 0) || (documents.size() == 0 ) || documents[0].creationDate <= date.minus(1) )
...
{
    helper.query("select d from FcntpDocumentBase d where d.ccamBOCase.code = 'documentAgreement' and d.parent.id = '" + contractUUID + "' order by d.creationDate DESC")
}